...
(() => {
  ...
  /** @type {Record<string, (message: string, ...args: unknown[]) => void>} */
  const logger = {
    error: (message, ...args) => {
  ...
  function getTitleElement(platform) {
    ...
    if (!element) {
      throw new Error(
        `Title element with selector "${selector}" not found for platform: ${platform}`,
      );
    }
    return element;
  }
  ...
  function getPRTitle(platform) {
    ...
    switch (platform) {
      ...
      case "github":
        return (
          titleElement.firstChild?.firstChild?.firstChild?.textContent?.trim() ||
          "Pull Request"
        );
    }
  }
  ...
  function getPRUrl(platform) {
    ...
    if (platform === "github") {
      ...
      if (!match) {
        throw new Error(`Invalid GitLab merge request path: ${pathname}`);
      }
      basePath = match[1];
    }
  ...
  function registerOptionsMenu() {
    ...
    const register = () => {
      menuId = GM_registerMenuCommand(
        ...
        () => {
          GM_setValue(
            INCLUDE_REPO_SHORTHAND_KEY,
            !shouldIncludeRepoShorthand(),
          );
